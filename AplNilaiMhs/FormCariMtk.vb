...
Public Class FormCariMtk
    ...
    Private Sub Isilist()
        Dim a As Integer
        Try
            Query = "SELECT * FROM tbl_matakuliah ORDER BY kdmtk"
    ...
    Private Sub caridatamhs()
        Dim a As Integer
        Try
            Query = "SELECT * FROM tbl_matakuliah Where kdmtk like '%" & Trim(txtcaridata.Text) & "%' or nmmtk like '%" & Trim(txtcaridata.Text) & "%'"
            daData = New MySqlDataAdapter(Query, Conn)
            dsData = New DataSet
            daData.Fill(dsData)
            ListView1.Items.Clear()
    ...
    Private Sub ambildatadarilistview()
        With ListView1.SelectedItems
            Try
                FormNilai.txtkdmtk.Text = .Item(0).SubItems(0).Text
                FormNilai.txtnamamtk.Text = .Item(0).SubItems(1).Text
    ...
    Private Sub FormCariMtk_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        KoneksikeDatabase()
        Posisilist()
        Isilist()
    End Sub
